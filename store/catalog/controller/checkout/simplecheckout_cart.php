...
class ControllerCheckoutSimpleCheckoutCart extends Controller { 
    ...
    public function index() {
        ...
        $this->data['button_order1']= $this->language->get('button_order');
								$this->data['continue_shopping']= $this->language->get('button_continue_shopping');
        ...
        foreach ($products as $product) {
            ...
            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                ...
                if(!isset($product['total_qty_discount'])){
                    $product['total_qty_discount']=0;
                }
                $qty = $product['quantity'] - $product['total_qty_discount'];
                $total = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $qty);
            } else {
                $total = false;
            }
            ...
            $this->data['products'][] = array(
                ...
                'total_qty_discount' => (isset($product['total_qty_discount'])?$product['total_qty_discount']:0),
                'stock'    => $product['stock'],
