...
class ControllerCheckoutSimpleCheckoutCustomer extends Controller {
    ...
    public function index() {
        ...
        if (!$this->error && ($asap || (!$asap && $this->data['simple_create_order']))) {
    ...
    public function update() {
        ...
        $this->simple->customer_info = array(
    ...
    private function validate() {
        ...
        if (!$this->customer->isLogged() && $register_customer) {
            if (!empty($email) && $this->model_account_customer->getTotalCustomersByEmail($email)) {
																		//$url = $this->url->link('checkout/simplecheckout_customer/login', '', 'SSL');
																		//$this->redirect($url);
																		
																		// changed by qasem qassem 2014-09-20 to prevent al-ready customer error email msg
                ...
                if ($register_customer) {
                    ...
                    if ($this->model_account_customer->getTotalCustomersByEmail($this->request->get['email'])) {
                        //$error = $this->language->get('error_exists');
																										// this cacneled just for checkout registeration page always give true osama request
                    }
                }
    ...
    private function save_customer_data() {
        if ($this->customer->isLogged()) {
            ...
            if (!empty($payment_address)) {
                ...
                $payment_address['simple']['address'] = $this->simple->get_custom_data(Simple::SET_CHECKOUT_CUSTOMER, Simple::OBJECT_TYPE_ADDRESS);
