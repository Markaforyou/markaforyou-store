...
class ControllerCheckoutSimpleCheckout extends Controller { 
    public function index() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            if ($this->validate() && ($asap || (!$asap && $this->data['simple_create_order']))) {                
                ...
                $this->simple->save_custom_data(Simple::SET_CHECKOUT_CUSTOMER, Simple::OBJECT_TYPE_ORDER, $order_id);
            ...
            $error_in = $this->simple->error;
            $error_in = !empty($error_in) ? $error_in : '';

            $this->data['simple_shipping_methods_hide'] = $error_in != 'shipping' ? $this->config->get('simple_shipping_methods_hide') : false;
            $this->data['simple_payment_methods_hide'] = $error_in != 'payment' ? $this->config->get('simple_payment_methods_hide') : false;
            $this->data['simple_customer_hide_if_logged'] = $this->customer->isLogged() && $error_in != 'customer' ? $this->config->get('simple_customer_hide_if_logged') : false;
    ...
    private function validate() {
        ...
        if (!empty($this->data['simple_common_view_agreement_checkbox']) && empty($this->data['agree'])) {
            $this->data['error_warning_agree'] = sprintf($this->language->get('error_agree'), $this->data['information_info']['title']);
